...
# Simplified constants - only the essential ID columns
ID_COLS = [
    "ZIP_CODE",
    "DELIVERY_SERVICE_CLASS",
    "DELIVERY_SERVICE_NAME",
    "ACCOUNT_IDENTIFIER",
    "INTERVAL_READING_DATE",  # Include this as it's always needed
]
...
def transform_wide_to_long(df: pl.DataFrame) -> pl.DataFrame:
    ...
    long_df = long_df.with_columns([
        # Extract HHMM from both formats:
        # "INTERVAL_HR0030_ENERGY_QTY" → "0030"
        # "INTERVAL_HR0030" → "0030"
    ...
    long_df = long_df.with_columns([
        (pl.col("hour_raw") // 24).alias("days_offset"),  # 1 if hour >= 24
        (pl.col("hour_raw") % 24).alias("hour"),          # 24→0, 25→1
    ])

    # Create datetime - IMPORTANT: Cast date to datetime first!
    long_df = long_df.with_columns([
        (pl.col("service_date").cast(pl.Datetime) +  # Cast to Datetime here
         pl.duration(
    ...
    # IMPORTANT: Remove rows where kwh is null (handles empty DST columns)
